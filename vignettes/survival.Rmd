...
vignette: >
  ...
  %\VignetteDepends{episodes, tidymodels, censored, survival, ggsurvfit, dplyr}
...
analysis using the Cox proportional hazards model with both fixed and
time-varying covariates. I will be using the simulated `substance_use`
...
## Death Analysis Across All Episodes
...
## Error Handling with episodes Functions

When using the `episodes` package functions, you can implement robust
error handling to better diagnose issues:
...
withCallingHandlers(
  ...
  error = function(e) {
    cli::cli_alert_danger("Error in segment_episodes: {conditionMessage(e)}")
    print(parent.frame())
  }
)
```
...
Key findings from our example analyses include:

-   Mental health disorders are associated with better treatment
    retention and lower mortality

-   Housing instability significantly increases both discontinuation and
    mortality risk

-   Treatment discontinuation is associated with nearly three times
    higher mortality risk

-   Opiate use is consistently associated with higher mortality compared
    to alcohol use

-   Medications like Naltrexone show protective effects against
    mortality
